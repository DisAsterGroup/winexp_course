...
<img src="./assets/img_0x0401.png" width="60%">
...
### API hashing
次に関数名を探す処理... といきたいところだが、`MessageBoxA`、`WriteFile`、`connect` などと直接関数名を埋め込むと、長さが異なるため比較がやり辛い。また、マルウェアの場合、文字列がそのまま埋め込まれていると解析されやすくなってしまう。そのため、しばしば関数名をハッシュ値に変換し、ハッシュ値同士で比較する API hashing というテクニックが用いられる。以下はそのハッシュ化する処理を実装したもの:
